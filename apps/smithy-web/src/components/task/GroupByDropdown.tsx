...
export function GroupByDropdown({
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div
          className="absolute z-20 mt-1 right-0 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-md shadow-lg py-1 min-w-40"
          ...
          {GROUP_BY_OPTIONS.map((option) => (
            <button
              ...
              className={`w-full text-left px-3 py-1.5 text-sm hover:bg-[var(--color-surface-hover)] flex items-center justify-between ${
                groupBy === option.value ? 'text-[var(--color-primary)] font-medium' : 'text-[var(--color-text)]'
              }`}
