---
title: Installation
description: Install and configure Stoneforge on macOS, Linux, or Windows. Set up agent providers like Claude Code, OpenCode, or OpenAI Codex for multi-agent orchestration.
---
...
<script type="application/ld+json" set:html={JSON.stringify({
  ...
  "@type": "HowTo",
  "name": "Install and configure Stoneforge",
  "description": "Install and configure Stoneforge on macOS, Linux, or Windows. Set up agent providers like Claude Code, OpenCode, or OpenAI Codex for multi-agent orchestration.",
  "totalTime": "PT5M",
  "tool": [
  ...
  "step": [
    {
      ...
      "text": "Run: npm install -g @stoneforge/smithy (or use pnpm, or build from source)"
    },
    {
      "@type": "HowToStep",
      "position": 2,
      "name": "Initialize your project",
      "text": "Navigate to a git repository and run: sf init"
    },
    {
      "@type": "HowToStep",
      "position": 3,
      "name": "Install an agent provider",
      "text": "Install at least one agent provider CLI such as Claude Code (npm install -g @anthropic-ai/claude-code), OpenCode, or OpenAI Codex"
    },
    {
      "@type": "HowToStep",
      "position": 4,
      "name": "Verify installation",
      "text": "Run sf doctor to check that everything is configured correctly"
    }
  ]
})} />
...
2. Initialize Stoneforge:
   ...
   <Aside type="tip">
     **Want to try Stoneforge without API keys?** Enable [demo mode](/guides/demo-mode/) after initialization, which uses a free model (opencode/minimax-m2.5-free) for all agents.
   </Aside>
</Steps>
...
<ParamTable params={[
  ...
  { name: 'base_branch', type: 'string', description: 'Base branch for merge targets (e.g., master or main). Auto-detected from git if omitted.' },
...
<LinkCard title="Stoneforge Overview" description="Learn more about how Stoneforge fits into your development workflow" href="https://stoneforge.ai" />
