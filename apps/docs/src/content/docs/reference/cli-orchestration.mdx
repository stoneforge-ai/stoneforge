...
If the task branch was already published by another path (for example a manual squash publish to `main`), `sf task merge` now reconciles that state too. When the branch tree already matches the target branch tree, Stoneforge treats it as already integrated, finishes closeout, and still performs best-effort cleanup.
...
When marking a task as `merged`, Stoneforge verifies either commit ancestry or final tree equivalence on `origin/<target>`. That lets already-published squash merges reconcile cleanly without `--force`. Reserve `--force` for cases where the branch history can no longer be verified, such as a deleted source branch.
