...
const docUpdateOptions: CommandOption[] = [
  ...
  {
    name: 'category',
    description: 'Document category (e.g., spec, prd, reference, other)',
    hasValue: true,
  },
];
...
const docUpdateCommand: Command = {
  ...
  help: `Update a document's content, metadata, and/or category, creating a new version when content changes.

Documents are versioned - each content update creates a new version while preserving
the history. Use 'sf document history' to view versions and 'sf document rollback' to
revert to a previous version. Category and metadata-only updates do not create a new version.
...
Options:
  ...
  -m, --metadata <json>  JSON metadata to merge
      --category <name>  New document category (e.g., spec, prd, reference, other)
...
Examples:
  ...
  sf document update el-doc123 --category other --metadata '{"customCategory": "known-issue"}'`,
