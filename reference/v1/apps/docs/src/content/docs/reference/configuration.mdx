...
<ParamTable title="General" params={[
  ...
  { name: 'baseBranch', type: 'string', description: 'Base branch for merge targets (e.g., master or main). Auto-detected from git if omitted.' },
  { name: 'database', type: 'string', default: 'stoneforge.db', description: 'SQLite database filename, relative to .stoneforge/.' },
  { name: 'demo_mode', type: 'boolean', default: 'false', description: 'Enable demo mode. When true, all agents use the free opencode/minimax-m2.5-free provider. Set automatically by sf init --demo or the dashboard settings toggle. See the Demo Mode guide for details.' },
]} />
...
<ParamTable title="External Sync" params={[
  { name: 'externalSync.enabled', type: 'boolean', default: 'false', description: 'Enable external sync with third-party tools.' },
  { name: 'externalSync.pollInterval', type: 'number', default: '60000', description: 'Background sync polling interval in ms (clamped to 10s–30min).' },
  { name: 'externalSync.conflictStrategy', type: 'string', default: 'last_write_wins', description: 'Conflict resolution strategy: last_write_wins, local_wins, remote_wins, or manual.' },
  { name: 'externalSync.defaultDirection', type: 'string', default: 'bidirectional', description: 'Default sync direction: push, pull, or bidirectional.' },
  { name: 'externalSync.autoLink', type: 'boolean', default: 'false', description: 'Auto-create external issues when new tasks are created.' },
  { name: 'externalSync.autoLinkProvider', type: 'string', description: 'Provider for task auto-linking (e.g., github, linear).' },
  { name: 'externalSync.autoLinkDocumentProvider', type: 'string', description: 'Provider for document auto-linking (e.g., notion, folder).' },
]} />
...
# Set default project (repo, team, database, or folder path)
sf external-sync config set-project github org/repo
sf external-sync config set-project folder ./docs
```

<Aside type="caution">
  Provider tokens are stored in the SQLite database, not in `config.yaml`. They are not committed to git and do not travel across branches.
</Aside>
