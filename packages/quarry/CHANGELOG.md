...
- @stoneforge/shared-routes@1.25.0

## 1.24.0

### Minor Changes

- c486f60: Add `--category` flag to `sf document update` so operators can change a document's category from the CLI. The flag is validated against the same category set as `sf document create` and can be combined with `--content`, `--file`, or `--metadata`.
