...
describe("TaskDispatchService edge cases", () => {
  ...
  it("requires repair and completes task records through explicit lifecycle methods", async () => {
    ...
    await service.runSchedulerOnce()
    const assignment = service.completeAssignment(
      service.listAssignments()[0].id
    )
    const repairRequired = service.requireTaskRepair(
      task.id,
  ...
  it("throws for missing records", () => {
    ...
    expect(() => service.getTask(asTaskId("missing"))).toThrow(
      /does not exist/i
    )
    expect(() =>
      service.getDispatchIntent(asDispatchIntentId("missing"))
    ).toThrow(/does not exist/i)
    expect(() => service.getAssignment(asAssignmentId("missing"))).toThrow(
      /does not exist/i
    )
