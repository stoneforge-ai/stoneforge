...
import { CodexAppServerJsonRpcConnection } from "./connection.js"
import {
...
async function runCodexAppServerTurn(
  ...
  return Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        const connection = yield* acquireCodexAppServerConnection(
          clientInput,
          turnInput
        )
...
function acquireCodexAppServerConnection(
  ...
  return Effect.acquireRelease(
    Effect.sync(() => {
      ...
      return new CodexAppServerJsonRpcConnection(process, turnInput.onEvent)
    }),
