...
describe("shared execution control-plane contract", () => {
  ...
  it("makes provider events readable while dispatch is still running", async () => {
    const workspaceId = makeWorkspaceId("workspace-streaming")
    const taskId = makeTaskId("task-streaming")
    const providerInstanceId = makeProviderInstanceId("claude-streaming")
    ...
    const controlPlane = createExecutionControlPlane({
      idSequence: {
        nextAgentId: () => makeAgentId("agent-generated"),
        nextAssignmentId: () => makeAssignmentId("assignment-streaming"),
        nextSessionId: () => makeSessionId("session-streaming"),
        nextTaskId: () => makeTaskId("task-generated")
      },
      providerInstances: [
        defineProviderInstance({
          ...
          provider: "claude-code",
          startSession: async (context) =>
            new Promise<ReturnType<typeof completedProviderResult>>(
              (resolve) => {
                context.onEvent?.({
                  kind: "provider.session.started",
                  providerSessionId: "claude-provider-session-streaming"
                })
                context.onEvent?.({
                  kind: "provider.session.completed",
                  status: "interrupted",
                  summary: "Provider is waiting for follow-up."
                })
                context.onEvent?.({
                  kind: "provider.log",
                  level: "info",
                  message: "provider accepted the task"
                })
                context.onEvent?.({
                ...
                completeProviderRun = () =>
                  resolve(
                    completedProviderResult(
                      context,
                      "claude-provider-session-streaming",
                      "Streaming provider completed."
                    )
                  )
              }
            )
    ...
    await configureClaudeWorkspace(controlPlane, workspaceId, {
      agents: [
        {
          acceptableRuntimes: [{ id: makeRuntimeId("runtime-test"), priority: 10 }],
          concurrencyLimit: 1,
    ...
    await controlPlane.createNoCodeTask({
      id: taskId,
      intent: "Prove live provider facts are readable.",
      requiredAgentTags: ["provider:claude-code"],
      title: "Stream provider facts",
      workspaceId
    })
    await controlPlane.activateTask({ taskId, workspaceId })

    const dispatch = controlPlane.dispatchNextTask({ workspaceId })

    await expect.poll(async () => {
      const snapshot = await controlPlane.readWorkspaceExecution({ workspaceId })

      return snapshot.sessions[0]?.logs[0]?.message
    }).toBe("provider accepted the task")
    await expect(controlPlane.readWorkspaceExecution({ workspaceId })).resolves
      .toMatchObject({
        sessions: [
          {
            finalSummary: "Provider is waiting for follow-up.",
            providerSessionId: "claude-provider-session-streaming",
            status: "running",
            transcript: [{ text: "first streamed update" }]
          }
        ]
      })
