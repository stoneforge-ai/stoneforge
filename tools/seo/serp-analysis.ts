  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const response = await fetch(
        `${baseUrl}/serp/google/organic/live/advanced`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        console.warn(
          `  ⚠ HTTP ${response.status} for "${keyword}" (attempt ${attempt}/${MAX_RETRIES}): ${body.slice(0, 200)}`
        )
      ...
      if (!postData.tasks || postData.tasks.length === 0) {
        console.warn(
          `  ⚠ No tasks returned for "${keyword}" (attempt ${attempt}/${MAX_RETRIES})`
        )
        if (attempt < MAX_RETRIES) {
          await sleep(RETRY_BASE_DELAY_MS * Math.pow(2, attempt - 1))
          continue
      ...
      if (taskResult.status_code !== 20000 || !taskResult.result) {
        console.warn(
          `  ⚠ Task failed for "${keyword}": status_code=${taskResult.status_code}, status_message="${taskResult.status_message ?? "unknown"}" (attempt ${attempt}/${MAX_RETRIES})`
        )
        if (attempt < MAX_RETRIES) {
          await sleep(RETRY_BASE_DELAY_MS * Math.pow(2, attempt - 1))
          continue
      ...
      console.warn(
        `  ⚠ Network error for "${keyword}" (attempt ${attempt}/${MAX_RETRIES}): ${err.message}`
      )
      if (attempt < MAX_RETRIES) {
        await sleep(RETRY_BASE_DELAY_MS * Math.pow(2, attempt - 1))
        continue
...
async function analyzeSerpForKeywords(
  keywords: string[]
  ...
  console.log(
    `Analyzing SERP data for ${total} keyword(s) (concurrency: ${SERP_CONCURRENCY})...`
  )
  ...
  for (let i = 0; i < total; i += SERP_CONCURRENCY) {
    ...
    const batchPromises = batch.map((keyword) =>
      fetchSerpForKeyword(keyword, baseUrl, auth)
    )
    ...
    console.log(
      `  Fetching SERP data: ${completed}/${total}${failed > 0 ? ` (${failed} failed)` : ""}`
    )
...
async function getKeywordDifficulty(
  ...
  if (!response.ok) {
    console.warn(
      `  Warning: Could not fetch keyword difficulty (${response.status})`
    )
  ...
  if (data.tasks) {
    for (const task of data.tasks) {
      if (task.status_code !== 20000 || !task.result) {
        console.warn(
          `  ⚠ Keyword difficulty task failed: status_code=${task.status_code}, status_message="${task.status_message ?? "unknown"}"`
        )
        continue
      }
      for (const result of task.result) {
        if (result.keyword) {
          difficultyMap.set(
            result.keyword,
            result.keyword_difficulty ?? result.competition ?? 0
          )
        }
      }
    }
...
async function main() {
  ...
  const avgDifficulty =
    serpData.length > 0
      ? serpData.reduce((sum, item) => sum + item.difficulty, 0) /
        serpData.length
      : 0
  const keywordsWithFeaturedSnippets = serpData.filter((item) =>
    item.serpFeatures.some((f) => f.type === "featured_snippet")
  ).length
  const keywordsWithPAA = serpData.filter((item) =>
    item.serpFeatures.some((f) => f.type === "people_also_ask")
  ).length
  ...
  if (serpData.length > 0) {
    ...
    for (const item of sorted.slice(0, 5)) {
      console.log(
        `  ${item.keyword} — difficulty: ${item.difficulty}, results: ${item.results.length}`
      )
    }
  }
}
