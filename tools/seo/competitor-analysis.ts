...
async function getBacklinkProfile(domain: string): Promise<BacklinkData> {
  ...
  if (!summaryResponse.ok) {
    ...
    throw new Error(
      `DataForSEO Backlinks Summary error (${summaryResponse.status}): ${body}`
    )
  }
  ...
  if (summaryTask?.status_code !== 20000) {
    console.warn(
      `  Warning: Backlinks summary for ${domain} returned status ${summaryTask?.status_code}: ${summaryTask?.status_message}`
    )
  }
  ...
  const referringResponse = await fetch(
    `${baseUrl}/backlinks/referring_domains/live`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (referringResponse.ok) {
    ...
    if (refTask?.status_code !== 20000) {
      console.warn(
        `  Warning: Referring domains for ${domain} returned status ${refTask?.status_code}: ${refTask?.status_message}`
      )
    }
    ...
    console.warn(
      `  Warning: Referring domains request for ${domain} failed (${referringResponse.status})`
    )
  }
...
async function findBacklinkGap(
  ...
  const response = await fetch(
    `${baseUrl}/backlinks/domain_intersection/live`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    console.warn(
      `  Warning: Domain intersection request failed (${response.status})`
    )
  ...
  if (intersectionTask?.status_code !== 20000) {
    console.warn(
      `  Warning: Domain intersection returned status ${intersectionTask?.status_code}: ${intersectionTask?.status_message}`
    )
  }
...
function computeManualGap(
  ...
  return gapDomains.sort(
    (a, b) => b.linksToCompetitors.length - a.linksToCompetitors.length
  )
}
...
async function main() {
  ...
  const competitors =
    args.domains && args.domains.length > 0 ? args.domains : DEFAULT_COMPETITORS
  ...
  for (const profile of profiles) {
    console.log(
      `  ${profile.domain}: ${profile.referringDomains} referring domains, rank: ${profile.domainRank}`
    )
  }
  console.log(`\nBacklink Gap:`)
  ...
  if (highValue.length > 0) {
    ...
    for (const d of highValue.slice(0, 10)) {
      console.log(
        `  ${d.domain} — links to: ${d.linksToCompetitors.join(", ")}`
      )
    }
  }
}
