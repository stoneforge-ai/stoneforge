...
async function fetchSearchAnalytics(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Search Analytics API error (${response.status}): ${errorBody}`
    )
  }
  ...
  for (const inspectionUrl of urls) {
    try {
      ...
      if (!response.ok) {
        const errorBody = await response.text()
        console.warn(
          `  Warning: Could not inspect ${inspectionUrl} (${response.status}): ${errorBody}`
        )
        results.push({
          url: inspectionUrl,
          verdict: "ERROR",
      ...
      console.warn(
        `  Warning: Error inspecting ${inspectionUrl}: ${err.message}`
      )
      results.push({
        url: inspectionUrl,
        verdict: "ERROR",
...
export function analyzeGSCInsights(
  ...
  const lowHangingFruit = queryPerformance
    .filter(
      (row) => row.position >= 5 && row.position <= 20 && row.impressions >= 10
    )
  ...
  const decliningPages = pagePerformance
    .filter(
      (row) => row.position <= 20 && row.ctr < 0.02 && row.impressions >= 20
    )
    .sort((a, b) => b.impressions - a.impressions)
    .slice(0, 20)
...
function calculateSummary(
  ...
  const totalClicks =
    queryPerformance.reduce((sum, r) => sum + r.clicks, 0) ||
    pagePerformance.reduce((sum, r) => sum + r.clicks, 0)
  const totalImpressions =
    queryPerformance.reduce((sum, r) => sum + r.impressions, 0) ||
  ...
  const avgPosition =
    queryPerformance.length > 0
      ? Math.round(
          (queryPerformance.reduce((sum, r) => sum + r.position, 0) /
            queryPerformance.length) *
            100
        ) / 100
      : pagePerformance.length > 0
        ? Math.round(
            (pagePerformance.reduce((sum, r) => sum + r.position, 0) /
              pagePerformance.length) *
              100
          ) / 100
        : 0
  ...
  const topQuery =
    queryPerformance.length > 0
      ? ([...queryPerformance].sort((a, b) => b.clicks - a.clicks)[0].query ??
        null)
      : null
  const topPage =
    pagePerformance.length > 0
      ? ([...pagePerformance].sort((a, b) => b.clicks - a.clicks)[0].page ??
        null)
      : null
...
async function fetchSiteData(
  ...
  if (args.inspect && args.inspect.length > 0) {
    ...
    if (siteInspectUrls.length > 0) {
      console.log(
        `  Inspecting ${siteInspectUrls.length} URL(s) for index coverage on ${siteUrl}...`
      )
      indexCoverage = await inspectUrls(accessToken, siteUrl, siteInspectUrls)
      if (indexCoverage) {
        console.log(
          `    Indexed: ${indexCoverage.indexedUrls}/${indexCoverage.inspectedUrls}`
        )
      }
    }
  }
...
function printSiteSummary(site: GSCSiteOutput, insights: GSCInsights): void {
  ...
  console.log(
    `  Total impressions: ${site.summary.totalImpressions.toLocaleString()}`
  )
  ...
  if (site.summary.topQuery)
    console.log(`  Top query: "${site.summary.topQuery}"`)
  ...
  if (site.indexCoverage) {
    ...
    for (const result of site.indexCoverage.results) {
      ...
      console.log(
        `    ${status} ${result.url} — ${result.coverageState} (${result.indexingState})`
      )
    }
  }
}
...
async function main() {
  ...
  const combinedInsights = analyzeGSCInsights(
    allQueryPerformance,
    allPagePerformance
  )
  ...
  for (const site of siteResults) {
    const siteInsights = analyzeGSCInsights(
      site.queryPerformance,
      site.pagePerformance
    )
  ...
  if (siteResults.length > 1) {
    ...
    console.log(
      `  Total clicks: ${combinedSummary.totalClicks.toLocaleString()}`
    )
    console.log(
      `  Total impressions: ${combinedSummary.totalImpressions.toLocaleString()}`
    )
