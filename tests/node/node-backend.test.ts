...
import {
  NodeStorageBackend,
  createNodeStorage,
} from "../../src/storage/node-backend.js"
  ...
  describe("In-Memory Database", () => {
    ...
    describe("SQL Execution", () => {
      ...
      it("should query with results", () => {
        ...
        const rows = backend.query<{ id: number; name: string }>(
      ...
      it("should queryOne with result", () => {
        ...
        const row = backend.queryOne<{ id: number; name: string }>(
    ...
    describe("Transactions", () => {
      ...
      it("should support savepoints", () => {
        ...
        const rows = backend.query<{ id: number }>(
          "SELECT * FROM test ORDER BY id"
        )
      ...
      it("should return value from transaction", () => {
        ...
        const result = backend.transaction((tx) => {
          tx.run("INSERT INTO test (id, name) VALUES (?, ?)", [1, "Alice"])
          return tx.queryOne<{ count: number }>(
