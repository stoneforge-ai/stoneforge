...
describe("Cross-Runtime Compatibility (Node.js)", () => {
  ...
  describe("SQL operations", () => {
    ...
    it("should insert and query data", () => {
      backend.run("INSERT INTO test (name, value) VALUES (?, ?)", [
        "alice",
        1.5,
      ])
    ...
    it("should run return lastInsertRowid", () => {
      ...
      const result = backend.run("INSERT INTO test (name) VALUES (?)", [
        "second",
      ])
    ...
    it("should handle UPDATE and return changes", () => {
      ...
      const rows = backend.query<{ value: number }>(
  ...
  describe("data type handling", () => {
    ...
    it("should handle text values with special characters", () => {
      ...
      const rows = backend.query<{ text_val: string }>(
        "SELECT text_val FROM types"
      )
    ...
    it("should handle empty strings", () => {
      ...
      const row = backend.queryOne<{ text_val: string }>(
        "SELECT text_val FROM types"
      )
  ...
  describe("prepared statements", () => {
    ...
    it("should prepare and get all rows", () => {
      const stmt = backend.prepare<{ name: string }>(
  ...
  describe("transactions", () => {
    ...
    it("should rollback failed transaction", () => {
      ...
      const row = backend.queryOne<{ balance: number }>(
    ...
    it("should support nested queries in transaction", () => {
      backend.transaction((tx) => {
        ...
        const count = tx.queryOne<{ cnt: number }>(
    ...
    it("should skip already applied migrations", () => {
      ...
      const migrations: Migration[] = [
        ...
        { version: 2, description: "Old", up: "SELECT 2" },
        {
          version: 3,
          description: "New",
          up: "CREATE TABLE new_table (id INT)",
        },
      ]
  ...
  describe("edge cases", () => {
    it("should handle very long strings", () => {
      ...
      const row = backend.queryOne<{ data: string }>(
        "SELECT data FROM long_text"
      )
