...
function testInternalImports() {
  const allowedInternalModules = new Set(["ids", "index", "models", ...publicModuleNames()]);
...
function publicModuleNames() {
  ...
  for (const indexPath of packageIndexPaths(root, codeRoots)) {
    publicModulesForIndex(indexPath).forEach((moduleName) => names.add(moduleName));
  }
...
function moduleMetric(filePath) {
  ...
  return {
    ...
    importExportRatio: nonCommentLines === 0 ? 0 : importExportLines / nonCommentLines,
  };
}
...
function ignoredPath(filePath) {
  return filePath.includes(`${path.sep}dist${path.sep}`) || filePath.includes(`${path.sep}coverage${path.sep}`);
}
